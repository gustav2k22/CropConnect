...
<div class="container mt-5">
    <div class="row">
        <?php
        // Query to fetch data from farmerregistration table with modified column names
        $query = "SELECT farmer_id, farmer_name, farmer_address FROM farmerregistration";
        $result = $con->query($query); // Assuming $conn is your existing database connection

        // Check if any records are available
        if ($result->num_rows > 0) {
            // Loop through each farmer's record and display details
                    ...
                    <div class="card border-dark border">
                        <div class="card-body">
                            <h5 class="card-title text-center">
                                <img src="iconbig<?php echo rand(1, 4); ?>.png" style="margin-bottom: 10px;">
                            </h5>
                            <h4 class="card-subtitle mb-2 text-center"><?php echo $row['farmer_name']; ?></h4>
                            <p class="card-text text-center">
                                <?php echo $row['farmer_address']; ?><br><br>
                                <button type="button" class="btn border-dark border" style="background-color:#FFD700;color:black" onclick="window.location.href='viewProfile.php?id=<?php echo $row['farmer_id']; ?>'">View Profile</button>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            echo "<p>No farmers found in the database.</p>";
        }
        ?>
    </div>
</div>
